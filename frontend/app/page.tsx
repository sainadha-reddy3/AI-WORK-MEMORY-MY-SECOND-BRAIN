...
type Filter = "all" | "today" | "week" | `type:${string}`;
...
export default function Home() {
  ...
  const counts = useMemo(() => {
    ...
    return { byType, today: todayCount, week: weekCount };
  ...
  const mode: "feed" | "answer" | "topic" = topic
    ? "topic"
    : answer
    ? "answer"
    : "feed";
    ...
    <div className="flex h-screen flex-col text-slate-200">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden w-60 shrink-0 flex-col overflow-y-auto border-r border-white/5 px-3 py-4 md:flex lg:w-64">
          ...
          {topics.length === 0 ? (
            <p className="px-2.5 text-xs text-slate-600">
              Topics appear as you record memories.
            </p>
          ) : (
            topics.map(([t, n]) => (
              <SidebarItem
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="px-8 pb-4 pt-7">
            <div className="mx-auto flex max-w-3xl items-end justify-between gap-4">
              <div className="min-w-0">
                ...
                <h2 className="font-display mt-1 truncate text-3xl text-white">
                  {title}
                </h2>
          ...
          <div className="px-8 pb-6">
            <div className="mx-auto max-w-3xl">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 shadow-2xl shadow-black/40 backdrop-blur-xl transition focus-within:border-indigo-400/40 focus-within:ring-4 focus-within:ring-indigo-500/10">
                <textarea
                  ref={composerRef}
                  rows={2}
                  value={input}
                  ...
                  placeholder="What did you work on? Just describe it naturally — I'll organise it."
                ...
                <div className="mt-2 flex items-center gap-1.5">
                  <ComposerChip icon="📎" label="Attach" />
                  <ComposerChip icon="🖼️" label="Screenshot" />
                  <ComposerChip icon="🎙️" label="Voice" />
                  <span className="ml-auto hidden text-[11px] text-slate-600 sm:inline">
                    Ctrl + Enter to save
                  </span>
                  <button
                    onClick={handleSave}
        ...
        <aside className="hidden w-72 shrink-0 flex-col gap-4 overflow-y-auto border-l border-white/5 p-4 lg:flex xl:w-80">
          {mode === "topic" ? (
            ...
            <Panel
              ...
              subtitle="From your last saved memory"
            >
      ...
      return (
        <div className="animate-fade-up rounded-2xl border border-white/[0.06] bg-white/[0.02] p-8 text-center">
          <p className="text-sm text-slate-400">
            Nothing recorded for {filterLabel(filter).toLowerCase()} yet.
          </p>
        </div>
      );
    }
    return (
      <div className="animate-fade-up flex flex-col items-center py-12 text-center">
        ...
        <p className="mt-2 max-w-sm text-sm text-slate-500">
          Describe what you worked on in your own words. I&apos;ll organise
          it — and I&apos;ll never invent anything you didn&apos;t say.
        </p>
      ...
      {answer.sources.length > 0 && (
        <div>
          <h3 className="mb-3 text-xs font-medium text-slate-400">
            ...
            {answer.sources.length === 1 ? "memory" : "memories"}
          </h3>
          <div className="space-y-3">
  ...
  if (history.total === 0) {
    return (
      <div className="animate-fade-up rounded-2xl border border-amber-400/20 bg-amber-500/[0.06] p-6">
        <p className="text-[15px] text-amber-100/90">
          You have no recorded memories about #{history.topic}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-up space-y-8">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard
          label="Unconfirmed"
          value={String(history.uncertain_count)}
          warn={history.uncertain_count > 0}
        />
      </div>
      ...
      {history.uncertain_count > 0 && (
        <p className="rounded-xl border border-amber-400/15 bg-amber-500/[0.05] px-4 py-3 text-xs leading-relaxed text-amber-200/90">
          {history.uncertain_count} of these {history.total} memories were
          recorded with uncertainty. Treat that part of this history as
          unconfirmed.
        </p>
      )}
            ...
            {history.by_type[t].map((m) => (
              <li key={m.id} className="relative">
                ...
                <div className="mt-1.5 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <TopicChips
                    topics={m.topics.filter((x) => x !== history.topic)}
        ...
        >
          ...
          <div className="mt-1 h-1 overflow-hidden rounded-full bg-white/5">
            <div
              ...
              style={{ width: `${(r.shared_memories / max) * 100}%` }}
            />
          </div>
        </button>
      ))}
      <p className="pt-2 text-[10px] leading-relaxed text-slate-600">
        Derived from your own tags — not guessed by AI.
      </p>
    </div>
  );
}
...
function AnswerInfo({ answer }: { answer: AskResponse }) {
  return (
    <div className="space-y-3 text-xs">
      ...
      <InfoRow
        label="Provider"
        value={`${answer.provider}${answer.provider_available ? "" : " (rules)"}`}
      />
      <p className="border-t border-white/5 pt-3 text-[11px] leading-relaxed text-slate-500">
        Answers use only memories you recorded. General knowledge is always
        labelled separately, and never presented as your experience.
      </p>
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      {preview.uncertainty_markers.length > 0 && (
        <p className="text-[11px] text-amber-300/80">
          Marked unconfirmed because you said &ldquo;
          {preview.uncertainty_markers.join('", "')}&rdquo;.
        </p>
      )}
      ...
      <p className="border-t border-white/5 pt-3 text-[10px] leading-relaxed text-slate-600">
        Title, type and topics are AI interpretation. Your original words are
        stored unchanged. Provider: {preview.provider}.
      </p>
    </div>
  );
}
...
function MemoryCard({ memory, onOpenTopic, showDate = false }: MemoryCardProps) {
  return (
    <article className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition hover:border-white/10 hover:bg-white/[0.035]">
      ...
      <h4 className="text-[15px] font-medium leading-snug text-slate-100">
        {memory.title}
      </h4>
      {titleDiffers(memory) && (
        <p className="mt-1.5 text-sm leading-relaxed text-slate-400">
...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ring-inset ${m.tone}`}
    >
      {m.icon} {m.label}
    </span>
  );
...
function StatCard({ label, value, warn = false }: StatCardProps) {
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
      ...
      <p className={`mt-1 text-sm font-medium ${warn ? "text-amber-300" : "text-slate-100"}`}>
        {value}
      </p>
    </div>
  );
...
function SidebarItem({
  ...
  return (
    ...
    >
      ...
      {count !== undefined && (
        <span
          className={`text-[11px] tabular-nums ${active ? "text-indigo-300" : "text-slate-600"}`}
        >
          {count}
        </span>
      )}
...
function SectionLabel({ text }: { text: string }) {
  return (
    <p className="mb-1 mt-6 px-2.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-600">
      {text}
    </p>
  );
}
...
function ComposerChip({ icon, label }: { icon: string; label: string }) {
  return (
    <button
      ...
      className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs text-slate-500 transition hover:bg-white/[0.04] disabled:cursor-not-allowed"
    >
      <span>{icon}</span>
...
function sourceLabel(m: Memory): string {
  const s = m.evidence[0]?.source_type;
  return s ? SOURCE_LABELS[s] ?? s : "No source";
}
